...
**Date:** 2026-09-11 (recheck: `/docs/` 200; Sample Page 404; LCP pass 13:47 BST)  
...
**Not yet launch-complete:** set Site Address at cutover; lab LCP still Poor (**7.2 s** `/`, **5.0 s** `/classes/` this run); live GSC still has `dev.` / `http://` / `www.` sitemaps; NAP/schema (JOHN DOE, 42 vs 43, SW8 1SR vs 1SS). Canonicals follow `home_url()`. Homepage FCP is now 1.8 s.
...
1. Lab LCP still Poor on `/` (**7.2 s**, overlay text) and `/classes/` (**5.0 s** this run). Fonts/Leaflet/calendar CSS/Ken Burns shipped; fail check “under 4 s” still fails.
...
4. Finish lab LCP: ClasbPro CSS/JS on drawer-open; inline `faces.css`; Scope Trial woff2 / delay H1 decode (`/` still 7.2 s).
...
- Staging: 752 sitemap URLs, 609 tutorials, Cloudflare AI-crawler blocks, no HSTS/CSP on HTML, lab LCP 7.2 s `/` / 5.0 s `/classes/` (FCP `/` 1.8 s).
...
| High | Lab LCP 7.2 s `/`, 5.0 s `/classes/` this run (`performance.md`) |
...
Staging mobile Lighthouse 13.4.1, Slow 4G, HTTP auth, **13:47 BST**: homepage LCP **7.2 s** (was 7.6), FCP **1.8 s** (was 3.6), `/classes/` LCP **5.0 s** (was 7.7; this run’s TTFB 1.3 s is an outlier). CLS 0.002 / 0.001 (good), TBT 180 / 50 ms (good). Fonts, Leaflet, calendar CSS, and Ken Burns slide-0 HTML have shipped. Remaining: overlay/H1 text LCP, ClasbPro CSS/JS on first paint, `faces.css` extra request, Scope Trial TTF. No field INP/CrUX (no API key). Live V7 TTFB ~0.95 s.
...
Homepage: 44 `<img>`, **0 missing alt**. Lab LCP on `/` is overlay copy (`p.font-body`), not the hero photo. Ken Burns slides 1–3 are in `<template>` and not fetched on first paint. `/tutorials-category/` inlines 391 images. Organization `logo` must not be a JPEG of a person.
