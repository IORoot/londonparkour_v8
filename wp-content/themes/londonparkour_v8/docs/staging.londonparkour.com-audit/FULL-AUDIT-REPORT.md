...
- Staging: 752 sitemap URLs, 609 tutorials, Cloudflare AI-crawler blocks, no HSTS/CSP on HTML, lab LCP 7.6–7.7 s.
...
Staging mobile Lighthouse 13.4.1, Slow 4G, HTTP auth: homepage LCP **7.6 s**, `/classes/` **7.7 s**, CLS 0.001 / 0.041 (good), TBT 170 / 90 ms (good). TTFB ~80–210 ms. Failure is render path (CSS/fonts/JS), not origin. No field INP/CrUX (no API key). Live V7 TTFB ~0.95 s.
...
Homepage: 44 `<img>`, **0 missing alt**. LCP is still a hero photograph. `/tutorials-category/` inlines 391 images. Organization `logo` must not be a JPEG of a person.
