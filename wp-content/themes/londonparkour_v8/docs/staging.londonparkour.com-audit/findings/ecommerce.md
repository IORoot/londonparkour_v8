...
|---|---|---|---|
| `/book/` | **301** → `/booking-cancelled/` | destination **noindex, nofollow** | WordPress `x-redirect-by`. Any leftover `/book/` link becomes a cancelled-booking page. |
| `/booking-cancelled/` | 200 | noindex, nofollow | Correct utility |
| `/booking-confirmed/` | 200 | noindex, nofollow | Correct utility |
| `/bookings/` | **404** | (404 noindex via theme) | Live V7 URL is indexed |
...
| `/giftcards/` | **404** | | Live V7 URL is indexed |
| `/gift-cards/` | 301 → `/docs/gift-cards/` | | |
| `/docs/gift-cards/` | 200 | index, follow | Docs/FAQ, H1 `Questions, answered.` — **no Offer/Product** |
| `/clasbpro-theme-preview/` | 200 | **index, follow** | In sitemap — should not be public |
| `/sample-page/` | 200 | **index, follow** | In sitemap |

Theme (`single-clasbpro_class.php`, `archive-clasbpro_class.php`): BOOK opens a **drawer**, not `/book/…` hrefs. Checkout is not a crawlable cart. That is the right model **if** `/book/` does not 301 to cancelled.
...
| `/book/` | Must not resolve to cancelled | **301 to noindex cancelled** — broken |
| `/clasbpro-theme-preview/` | **No** | index + sitemap |
| Live `/bookings/` | Historically a booking UI | Still indexed on V7; 404 on V8 |

404s are noindex via `lp_seo_is_noindex()`. Theme QA slugs `blocks-qa`, `booking-*` are excluded from sitemaps — preview and sample-page are **not**.
...
2. The booking **IA is drawer-based**, so there is no public checkout to noindex — except `/book/` which currently poisons the old path.
3. Gift cards **lost a product URL**. Live `/giftcards/` must 301 to a real purchase surface or to `/docs/gift-cards/` if gift cards are no longer sold. Docs-only will drop gift-card queries (`parkour gifts` 1 click / 15 impr / pos 11.1 on live `/giftcards/`).
...
| Severity | Finding |
|---|---|
| Launch blocker | `/book/` 301 → `/booking-cancelled/` (noindex) |
| Launch blocker | Live indexed `/bookings/` and `/giftcards/` have no V8 equivalents (404 / docs) |
| High | `/clasbpro-theme-preview/` and `/sample-page/` indexable + in sitemap |
...
1. Map live `/bookings/` → `/classes/` (or coupons), `/giftcards/` → the real gift-card buy URL or `/docs/gift-cards/`. Fix `/book/` so unauthenticated hits 404 or `/classes/`, **never** cancelled.
2. `noindex` + drop from sitemap: `clasbpro-theme-preview`, `sample-page`. Confirm `blocks-qa` stays out.
