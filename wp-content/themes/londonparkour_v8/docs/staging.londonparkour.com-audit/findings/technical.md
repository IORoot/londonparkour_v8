...
| Core Web Vitals | fail | 38 | Lab mobile LCP **7.8 s** `/`, **6.6 s** `/classes/` this run (`performance.md`). FCP `/` 2.3 s. CLS good. No field INP. `/tutorials-category/` is 1.48 MB HTML. |
...
2. **Lab LCP 7.8 s `/`, 6.6 s `/classes/`** this run (`performance.md`). Render-blocking CSS gone; still Poor. Field CWV does not exist for this host.
