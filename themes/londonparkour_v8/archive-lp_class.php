<?php
/**
 * archive-lp_class.php — ClassesListings.
 *
 ...
 *
 * `is_featured` on lp_class, new here, mirroring `is_lead` on Coach and
 * `is_flagship` on Location for the reason PORT-FINDINGS §13 records: the page
 * shows one class above the grid and the rest below, so the grid query must
 * know which is already on show or it renders twice. No ordering picks it —
 * the design's featured class is neither first nor newest.
 *
 ...
 *
 * ── The four spec facts, and the one that does not fit ─────────────────────
 *
 * PRICE, DURATION and RUNS all come from real fields. DURATION is the first
 * ` · ` segment of `subtitle`, which is that field's own documented format
 * ("60 min · all kit provided" — see its instructions in acf-groups.php).
 ...
 *
 * **SITES does not fit the data model.** The design reads `6`, but a class here
 * has ONE `location` (an ACF post_object), so the honest count is 1. It is
 * rendered as counted rather than hardcoded to the design's number, and the
 * gap is reported in PORT-FINDINGS — the fix is a `locations` relationship
 * field, which is a data-model decision for the repo owner, not a port
 * decision. Do not paper over it with a literal 6.
 *
 * ── Filtering ──────────────────────────────────────────────────────────────
 *
 * The filter grid is a real GET form. See app/setup/queries.php for why the
 * parameters are `class_search`/`class_level`/`class_site` and not `s` and the
 * taxonomy query var, and parts/components/filter-grid.php for how the design's
 * missing submit control is handled without inventing one.
 *
...
/** The first ` · ` segment of a class subtitle is its duration, by that field's own format. */
$lp_duration = static function ( int $lp_id ): string {
...
$lp_card = static function ( WP_Post $lp_post ) use ( $lp_duration ): array {
	$lp_levels   = get_the_terms( $lp_post, 'lp_level' );
	$lp_location = function_exists( 'get_field' ) ? (int) get_field( 'location', $lp_post->ID ) : 0;
	$lp_price    = function_exists( 'get_field' ) ? (string) get_field( 'price', $lp_post->ID ) : '';
	$lp_dur      = $lp_duration( $lp_post->ID );
...
$lp_grid = array_values(
	array_filter(
		$lp_classes,
		static fn( WP_Post $lp_p ): bool => ! $lp_featured || $lp_p->ID !== $lp_featured->ID
	)
);
...
<main id="main">
	...
	<?php
	if ( $lp_featured ) :
		$lp_f_id       = $lp_featured->ID;
		$lp_f_levels   = get_the_terms( $lp_featured, 'lp_level' );
		$lp_f_level    = is_array( $lp_f_levels ) && $lp_f_levels ? $lp_f_levels[0]->name : '';
		$lp_f_location = function_exists( 'get_field' ) ? (int) get_field( 'location', $lp_f_id ) : 0;
		$lp_f_price    = function_exists( 'get_field' ) ? (string) get_field( 'price', $lp_f_id ) : '';
		$lp_f_p_label  = function_exists( 'get_field' ) ? (string) get_field( 'price_label', $lp_f_id ) : '';
		$lp_f_sessions = function_exists( 'get_field' ) ? get_field( 'sessions', $lp_f_id ) : null;
		$lp_f_sessions = is_array( $lp_f_sessions ) ? $lp_f_sessions : array();
		$lp_f_first    = $lp_f_sessions[0] ?? array();
		$lp_f_action   = function_exists( 'get_field' ) ? lp_action( get_field( 'primary_action', $lp_f_id ) ) : null;
		$lp_f_thumb    = get_post_thumbnail_id( $lp_featured ) ?: 0;
				...
				<div class="w-full lg:w-[380px] lg:shrink-0">
					...
					lp_part(
						...
						array(
							...
							'cta_label'  => $lp_f_action['label'] ?? 'BOOK THIS SESSION',
							'href'       => $lp_f_action['href'] ?? '',
